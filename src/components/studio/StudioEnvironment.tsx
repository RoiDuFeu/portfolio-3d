...
function StarLayer({
  ...
  const [positions, colors, sizes, offsets, brightnesses] = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const brightness = brightnessRange[0] + rawBright * (brightnessRange[1] - brightnessRange[0])
      brightnesses[i] = brightness

      const tempBias = brightness * 0.6 + Math.random() * 0.4
      const temp = tempRange[0] + tempBias * (tempRange[1] - tempRange[0])
      const c = starColor(temp)
      colors[i3] = c.r
      colors[i3 + 1] = c.g
      colors[i3 + 2] = c.b

      sizes[i] = sizeRange[0] + brightness * (sizeRange[1] - sizeRange[0]) + Math.random() * 0.5
...
export function StudioEnvironment() {
  return (
    <>
      {/* Background dim stars — dense faint carpet */}
      <StarLayer
        count={8000}
        radiusMin={120}
        radiusMax={250}
        ...
        tempRange={[2500, 6000]}
      />
      {/* Mid-field stars — moderate brightness */}
      <StarLayer
        count={2500}
        radiusMin={70}
        radiusMax={140}
        ...
        tempRange={[3000, 8000]}
      />
      {/* Foreground bright stars — sparse, vivid, colorful */}
      <StarLayer
        count={500}
        radiusMin={40}
        radiusMax={100}
